...
export class ClaudeAgentService {
  ...
  /** 处理命令行输入（/help、/clear、/model...）；返回 true 表示已消费 */
  async handleCliCommand(input: string): Promise<boolean> {
    ...
    if (command === 'help') {
      this.pushSystemMessage([
        'ZyTrader CLI Commands:',
        '/help  显示帮助',
        '/clear 清空会话',
        '/mode <single|plan|parallel> 切换协调模式',
        '/plan <step1;step2;...> 生成计划步骤',
        '/model <name> 切换当前模型',
        '/status 查看会话状态',
      ].join('\n'));
      this.appendLog({ toolName: 'cli', action: 'help', result: 'success' });
      return true;
    }

    if (command === 'clear') {
      await this.clearSession();
      this.pushSystemMessage('会话已清空。');
      this.appendLog({ toolName: 'cli', action: 'clear', result: 'success' });
      return true;
    }

    if (command === 'mode') {
      const modeArg = (args[0] ?? '').toLowerCase();
      if (modeArg === 'single' || modeArg === 'plan' || modeArg === 'parallel') {
        this.setPlanMode(modeArg);
        this.pushSystemMessage(`已切换模式: ${modeArg}`);
        return true;
      }
      this.pushSystemMessage('用法: /mode <single|plan|parallel>');
      return true;
    }

    if (command === 'plan') {
      const planText = args.join(' ').split(';').map((x) => x.trim()).filter(Boolean).join('\n');
      if (!planText) {
        this.pushSystemMessage('用法: /plan step1;step2;step3');
        return true;
      }
      this.generatePlanFromText(planText);
      this.pushSystemMessage('计划步骤已生成。');
      return true;
    }
    ...
    if (command === 'model') {
      ...
      if (!model) {
        this.pushSystemMessage(`当前模型: ${this.runtime.client.getModel().model}`);
        return true;
      }
      const current = this.runtime.client.getModel();
      this.runtime.client.configureRuntime({
        model: {
          ...current,
          model,
      ...
      this.pushSystemMessage(`模型已切换为: ${model}`);
      this.appendLog({ toolName: 'cli', action: `model:${model}`, result: 'success' });
      return true;
    }

    if (command === 'status') {
      const mode = this.planModeSubject.value;
      const msgCount = this.messagesSubject.value.length;
      const toolCount = this.toolsSubject.value.length;
      this.pushSystemMessage(`status => mode=${mode}, messages=${msgCount}, tools=${toolCount}`);
      return true;
    }

    this.pushSystemMessage(`未知命令: /${command}，输入 /help 查看可用命令。`);
    this.appendLog({ toolName: 'cli', action: `unknown:${command}`, result: 'failed' });
    return true;
  }
