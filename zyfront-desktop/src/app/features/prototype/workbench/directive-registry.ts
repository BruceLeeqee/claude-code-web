...
export const DIRECTIVE_REGISTRY: DirectiveDefinition[] = [
  ...
  {
    name: '/team-struct',
    desc: '管理协作结构（new/confirm/reject/list/info）',
    template: '/team-struct <new|confirm|reject|list|info> [args]',
    kind: 'team_struct',
    group: 'development',
    usage: '/team-struct new "协作流程"  /team-struct confirm <名称>  /team-struct reject <名称>  /team-struct list  /team-struct info <结构名>',
    visibleInHelp: true,
    requiresArgs: true,
  },
  {
    name: '/team-run',
    desc: '按协作结构启动团队执行（加 -b 开启头脑风暴）',
    template: '/team-run <struct> <task> [-b]',
    kind: 'team_run',
    group: 'development',
    usage: '/team-run <结构名> <团队任务> [-b]',
    visibleInHelp: true,
    requiresArgs: true,
  },
  {
