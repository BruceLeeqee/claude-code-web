...
export class ChatPageComponent implements OnInit, AfterViewChecked, OnDestroy {
  ...
  readonly showCommandPalette = signal(true);

  private readonly commandList = ['/help', '/clear', '/mode', '/plan', '/model', '/status'];
...
<div class="code-block" data-language="${lang}">
  ...
  /** Enter 发送，Shift+Enter 换行；Tab/上下键支持命令体验 */
  onComposerKeydown(event: KeyboardEvent): void {
    if (event.key === 'Tab') {
      const current = this.input().trim();
      if (current.startsWith('/')) {
        event.preventDefault();
        const next = this.commandList.find((cmd) => cmd.startsWith(current));
        if (next) this.input.set(`${next} `);
      }
      return;
    }
    ...
    if (event.key === 'ArrowUp') {
      ...
      event.preventDefault();
      this.historyCursor = this.historyCursor < this.history.length - 1 ? this.historyCursor + 1 : this.historyCursor;
      const idx = this.history.length - 1 - this.historyCursor;
      this.input.set(this.history[idx] ?? this.input());
      return;
    }
    ...
    if (event.key === 'ArrowDown') {
      if (this.history.length === 0) return;
      event.preventDefault();
      this.historyCursor = this.historyCursor > 0 ? this.historyCursor - 1 : -1;
      if (this.historyCursor === -1) {
        this.input.set('');
      } else {
        const idx = this.history.length - 1 - this.historyCursor;
        this.input.set(this.history[idx] ?? '');
      }
      return;
    }
  ...
  async sendStream(): Promise<void> {
    ...
    const handled = await this.agent.handleCliCommand(text);
    if (handled) {
      this.scheduleScrollThread();
      return;
    }
  ...
  /** 根据输入返回命令建议列表 */
  get commandSuggestions(): string[] {
    const v = this.input().trim();
    if (!v.startsWith('/')) return [];
    return this.commandList.filter((cmd) => cmd.startsWith(v)).slice(0, 6);
  }
