/**
 * 将 workbench.page.ts 中占位乱码替换为中文（UTF-8 一次性写回）。
 * 运行：node scripts/apply-workbench-zh.mjs
 */
...
const p = path.join(__dirname, '../src/app/features/prototype/workbench/workbench.page.ts');
let s = fs.readFileSync(p, 'utf8').replace(/\r\n/g, '\n');

const pairs = [
  [
    '/** ?????? localStorage?v2 ? transcript ?????? */',
  ...
  [
    `        const body =
        ...
        this.aiXtermWrite(\`\\x1b[35m[??]\\x1b[0m\\r\\n\`);`,
    `        const body =
          text.length > REPLAY_ASSISTANT_MAX ? \`\${text.slice(0, REPLAY_ASSISTANT_MAX)}\\n…\` : text;
  ...
  [
    '      this.aiXtermWrite(`\\x1b[36m[??]\\x1b[0m ${toolPreview.replace(/\\r?\\n/g, \' \')}\\r\\n`);',
  ...
  ['const title = trimmed.length > 56 ? `${trimmed.slice(0, 53)}?` : trimmed;', 'const title = trimmed.length > 56 ? `${trimmed.slice(0, 53)}…` : trimmed;'],
...
for (const [a, b] of pairs) {
  if (!s.includes(a)) {
    console.error('MISSING:', a.slice(0, 100).replace(/\n/g, '\\n'));
    process.exit(1);
  }
  s = s.split(a).join(b);
}
